...
public class CharacterMotor : MonoBehaviour
{
	...
	void Awake()
	{
		networkViewer = gameObject.GetComponent<NetworkView>();
	...
	private void UpdateFunction()
	{
		...
		if(!controller.enabled || (networkViewer && (ID != UnitZ.gameManager.PlayerID && !networkViewer.isMine) && (Network.isServer || Network.isClient))){
			IsActive = false;
			return;
		}
		...
		if (MoveWithPlatform())
		{
			...
			if (moveDistance != Vector3.zero){
				VelocityDirection = moveDistance;
		...
		VelocityDirection = currentMovementOffset;
