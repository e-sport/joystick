...
public class PlayerSave : MonoBehaviour
{
	...
	public void LoadPlayer (CharacterSystem character)
	{
		...
		string hasKey = UnitZ.gameManager.UserID + "_" + Application.loadedLevelName +"_"+ character.CharacterKey + "_" + UnitZ.gameManager.UserName;
	...
	public void InstantiateCharacter (CharacterSystem character, string hasKey)
	{
		...
		if ((!Network.isServer && !Network.isClient) || Network.isServer) {
			...
			if (Network.isServer) {
				if (character.networkViewer)
					character.networkViewer.RPC ("ApplyData", RPCMode.Others, PlayerSaveDataText (playersave));
			}
		}
	}
	...
	{
		string hasKey = playersave.UID + "_" + Application.loadedLevelName +"_"+ playersave.CharacterKey + "_" + playersave.PlayerName;
